...
class CustomTransform:
    ...
    def __call__(self, image_cv):
        if image_cv is None or not isinstance(image_cv, np.ndarray):
            raise ValueError("Invalid image input to CustomTransform")
        ...
        # Convert to tensor
        # Convert color from BGR to RGB, normalize if necessary
        image_cv = cv.cvtColor(image_cv, cv.COLOR_BGR2RGB)
        image_tensor = torch.from_numpy(image_cv).permute(2, 0, 1).float() / 255.0
